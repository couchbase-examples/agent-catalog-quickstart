...
class RouteDataLoader:
    ...
    def load_route_data(self) -> List[str]:
        ...
        try:
            ...
            from data.route_data import get_travel_knowledge_base
...
class CouchbaseSetup:
    ...
    def setup_bucket_scope_collection(self):
        ...
        try:
            ...
            try:
                self.cluster.query(f"CREATE PRIMARY INDEX IF NOT EXISTS ON `{bucket_name}`.`{scope_name}`.`{collection_name}`").execute()
    ...
    def setup_vector_search_index(self):
        ...
        try:
            ...
            if os.path.exists(index_file):
                ...
                scope_index_manager = self.cluster.bucket(os.environ["CB_BUCKET_NAME"]).scope(os.environ["SCOPE_NAME"]).search_indexes()

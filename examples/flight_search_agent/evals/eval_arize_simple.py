...
class SimpleFlightEvaluator:
    ...
    def run_evaluation(self, test_queries: List[str]) -> pd.DataFrame:
        ...
        for i, query in enumerate(test_queries, 1):
            ...
            # Log summary
            if result['success']:
                logger.info(f"✅ Score: {evaluation['completeness_score']:.1f}% | Type: {evaluation['query_type']}")
                logger.info(f"📊 Features: Flight({evaluation['has_flight_info']}) Book({evaluation['has_booking_info']}) Policy({evaluation['has_policy_info']})")
            else:
    ...
    def _print_summary(self, results_df: pd.DataFrame, phoenix_started: bool):
        ...
        for query_type, count in query_types.items():
            avg_score = results_df[results_df['query_type'] == query_type]['completeness_score'].mean()
        ...
        for feature in features:
            ...
            logger.info(f"  {feature.replace('has_', '').replace('_', ' ').title()}: {count}/{total}")

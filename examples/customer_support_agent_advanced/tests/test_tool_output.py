    ...
    try:
        ...
        for source, dest, description in test_routes:
            ...
            try:
                ...
                result = tool_result.func({"source_airport": source, "destination_airport": dest})
...
def test_python_tools():
    ...
    python_tools = ["update_customer_context", "get_customer_insights"]
        ...
        for tool_name in tool_names:
            try:
                ...
                if tool:
                    tool_type = (
                        "SQL++"
                        if tool_name.endswith("_info") or tool_name.endswith("_base")
                        else "YAML"
                        if tool_name.endswith("_policies")
                        else "Python"
                    )

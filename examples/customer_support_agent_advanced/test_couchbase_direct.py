...
def test_flight_lookup(source_airport: str, destination_airport: str):
    ...
    try:
        ...
        for i, flight in enumerate(flights, 1):
            print(f"{i}. {flight['airline']} - {flight['equipment']} - {flight['distance']} miles - {flight['flight_type']}")
            print(f"   {flight['source_name']} ({flight['source_city']}) → {flight['dest_name']} ({flight['dest_city']})")
